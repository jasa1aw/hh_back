...
const createApply = async (req, res) => {
    try {
        ...
        sendMail(user.email, `Новый отклик на ваканисию ${vacancy.name}`, `
...
const declineEmployee = async (req, res) => {
    try {
        ...
        sendMail(user.email, `Отказ на вакансию ${vacancy.name}`, `
            Компания: ${company.name},  к сожелению ваша кондидатура не подкходит на вакансию ${vacancy.name}
        `)
        res.status(200).end()
    } catch(error) {
        res.status(500).send(error)
